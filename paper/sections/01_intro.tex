...
equations (DDEs)~\cite{stepan1989retarded,hale1993introduction,michiels2014stability}.
...
Rekasius substitution~\cite{rekasius1980stability}, and the cluster treatment
of characteristic roots~\cite{olgac2002exact}; the Lambert-$W$
formalism~\cite{yi2010timedelay} and the matrix method of
Louisell~\cite{louisell2001matrix} target the dominant roots directly, while
the quasi-polynomial root finder QPmR~\cite{vyhlidal2009mapping,vyhlidal2014qpmr}
computes all roots in a prescribed rectangle from the values of
$\Dfun(\lam)$ on a grid. A recent survey of argument-principle-based
counting techniques is given in~\cite{niculescu2023counting}.
...
are \emph{not} the stability boundaries: they merely mark root crossings, and
the number of unstable roots on either side remains unknown. If the system is
well understood, the stable side can be picked by inspection or by a few test
computations, but automating this decision is surprisingly hard: the curves
must be assembled into closed cells by triangulation, numerical noise
generates many spurious slivers that must be filtered, and the bookkeeping
often costs more than the curve tracing itself. The turning stability-lobe
diagram illustrates a second, more fundamental inefficiency: towards low
spindle speeds the lobe structure becomes infinitely dense, and almost all
D-curves lie deep inside the unstable domain
(cf.~\ref{app:gallery})~\cite{stepan2001modelling,insperger2003stability,bachrathy2013improved},
...
a fast and robust tool with \emph{interpolable} output. The contributions are the
...
root -- is estimated \emph{during the same sweep} at negligible extra cost, and
can be refined to machine precision by a few Newton corrections for a few
percent of the cost; tracking several local minima yields multiple roots at
once. This ordered march, and not accuracy or speed, is what distinguishes
the phase ODE from plain quadrature: a recursively subdividing quadrature
...
and Section~\ref{sec:conclusion} concludes. \ref{app:gallery} collects twelve
