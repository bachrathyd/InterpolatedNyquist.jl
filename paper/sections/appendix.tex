...
stabilized elsewhere. (One qualification from Section~\ref{sec:tracking}
applies: the gallery sweeps track a single $|\Dfun|$ minimum per point, so
deep inside the stable domain the colour may reflect a non-dominant root
branch; near the boundary -- the region the charts are read for -- the
crossing root always produces the global minimum and the shading is
faithful.) The black curve is the stability boundary traced by
