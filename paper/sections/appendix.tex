...
it is a poor one: it grows like $\lam^{58}$, reaches magnitudes of $10^{298}$
before overflowing, and forces the counting formula onto an estimate of a very
large leading order -- which leaves an integer residual near $0.4$, a rounded
count barely better than a coin toss. Factoring out the open-loop dynamics,
...
hence $n=0$ exactly and a residual of $\sim10^{-7}$. Because the feedback
...
determinant by one solve with a single right-hand side: identical to machine
precision and $5.5$ times faster per evaluation, which is what makes this
panel affordable at the gallery's resolution. For multi-input feedback of
