...
modes are damped ever harder, $\Real\lam_k\to-\infty$ (here
$-0.06,-0.56,-1.54,-3.02,\dots$), only finitely many roots sit near the axis,
and $|\operatorname{sech}(\gamma L)|$ decays like $\ee^{-\sqrt{\omega/2c}}$.
The count is then well posed, and the chart over $(K_p,\tau)$ in
Fig.~\ref{fig:galleryb} shows the resulting stable bands with an integer
residual of $\sim10^{-7}$ (the physically wrong viscous model gives $6368$
...
\eqref{eq:returndiff} reduces to the classical scalar return difference
$1+L(\lam)$ used in~\eqref{eq:beam}; for multi-input feedback the
determinant form remains valid, at the cost of one extra LU solve per
evaluation. One caveat is inherited from classical loop analysis: an exact
