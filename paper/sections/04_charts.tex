...
\begin{equation}\label{eq:coloring}
  C(\vect p) =
  \begin{cases}
    \sest(\vect p), & \text{if } \Zint(\vect p) = 0,\\[2pt]
    \Zint(\vect p), & \text{otherwise,}
  \end{cases}
\end{equation}
...
crossing within each bracketing cell to sub-cell accuracy. One detail is
essential rather than incidental: $\sest$ here must be the \emph{dominant}
root of Section~\ref{sec:tracking} (several tracked minima, maximal real
part), not the single closest minimum. The latter jumps where one branch
overtakes another, and a linear model fitted inside a cell straddling that
jump interpolates a zero that does not exist -- a phantom boundary point in
the interior of the stable domain, which is invisible in the traced curve but
corrupts anything built on the point cloud, such as the inscribed circle of
Section~\ref{sec:robust}. Note the contrast
...
mesh trace the boundary at a $320\times320$-equivalent resolution from about
$3500$ evaluations (Table~\ref{tab:mdbm}) -- some $30$ times fewer than the
$10^{5}$ a brute-force grid of that density would need.
