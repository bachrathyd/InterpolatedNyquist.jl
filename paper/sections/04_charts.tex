...
root of Section~\ref{sec:tracking}, not the single closest minimum -- the
latter is discontinuous at branch crossings, and a linear model fitted
inside a cell straddling the jump interpolates a phantom boundary point in
the interior of the stable domain, corrupting anything built on the point
cloud, such as the inscribed circle of Section~\ref{sec:robust}. Note the contrast
...
mesh trace the boundary at a $320\times320$-equivalent resolution from
under $2000$ evaluations (Table~\ref{tab:mdbm}) -- some $50$ times fewer
than the $10^{5}$ a brute-force grid of that density would need.
