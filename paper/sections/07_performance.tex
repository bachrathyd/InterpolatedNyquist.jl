...
separates them because conflating them is what makes performance claims
about stability charts unfalsifiable. With the \emph{safe} settings --
$\wmax=10^{6}$ and $\mathrm{tol}=10^{-5}$ -- a complete $100\times100$ chart
of a scalar characteristic function is a matter of seconds and every count
matches a tight reference. With an \emph{exploratory} preset --
$\wmax=10^{4}$, $\mathrm{tol}=10^{-4}$ -- the same grid resolves in a
fraction of that, at the price of a handful of boundary-adjacent counts
(Section~\ref{sec:showcase-tolerance}); the traced boundary, which is built
from $\sest$ rather than from the count, is unaffected. Neither setting is
``the'' performance of the method: the first is what to quote for a
published chart, the second for interactive exploration.
