...
right of the axis, so the sign test of Section~\ref{sec:integerness} exposes
the inconsistency for free. Since that test is available only from the
...
Table~\ref{tab:evals} isolates what actually drives the cost, and corrects an
impression the previous paragraphs might leave. The decisive quantity is not
the back-end but the decay rate of the phase ripple, which is set by how far
the delayed terms sit below the leading power of $\Dfun$. Where the ripple
decays fast -- delayed \emph{position} feedback, $\theta'\sim\omega^{-3}$ --
quadrature recognizes a negligible tail panel by panel and stops after a few
dozen evaluations even for $\wmax=10^{6}$, an order of magnitude below what
the march spends walking there. (One implementation detail matters for
honesty here: the quadrature is seeded with logarithmically spaced interior
breakpoints, one per two frequency decades. Without them its \emph{first}
panel spans the whole window, the lowest node lands above the entire
resonance region, and the fast-decaying tail looks converged after a single
panel -- returning $\Zraw=n/2$ with a \emph{perfect} integer residual, a
silently wrong count that the self-diagnostic cannot flag. The breakpoints
cost a few dozen extra evaluations and remove the failure mode.) Where the
ripple decays slowly -- delayed \emph{velocity} feedback,
$\theta'\sim\omega^{-1}$ -- both back-ends must resolve the whole range and
land within a factor of two of each other. Neither back-end is ever an order
of magnitude worse than the other on the count at the production setting
$\wmax=10^{4}$. The lesson for a user is to spend attention on $\wmax$ and on
the structure of the feedback, not on the choice of integrator.
