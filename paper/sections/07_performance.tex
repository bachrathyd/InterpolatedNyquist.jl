...
The showcase DDAE in the same table is the instructive entry, and it carries
the paper's sharpest practical lesson. Its accurate chart takes \emph{fifty
minutes}, its exploratory chart \emph{forty-eight seconds} -- a factor of
sixty for three miscounted pixels out of ten thousand. The reason is the
compounding of two costs that Section~\ref{sec:phaseode} treats separately:
this system's delayed \emph{velocity} feedback makes the phase ripple decay
only like $\omega^{-1}$, so raising $\wmax$ from $10^{4}$ to $10^{6}$ forces
the march to resolve two further decades of it, and every one of those
evaluations runs a $7\times7$ dual-number determinant through the automatic
extraction. Neither factor is a defect of the counting method; both are
choices the user makes. And the correct choice here is the cheap one: the
tail truncated at $\wmax=10^{4}$ contributes $\approx0.2/\wmax=2\cdot10^{-5}$
to $\Zraw$, five orders of magnitude below the rounding threshold, so the
expensive chart buys nothing but certainty about pixels that were already
right. This is what the asymmetric guidance of Section~\ref{sec:phaseode}
means in practice -- be generous with $\wmax$ when the ripple decays fast,
where it is nearly free, and frugal when it does not.
...
Section~\ref{sec:alternatives}. For the showcase DDAE every evaluation runs
...
costs roughly two orders of magnitude against a hand-coded characteristic
