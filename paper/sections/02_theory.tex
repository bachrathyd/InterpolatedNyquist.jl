...
\paragraph{Fractional orders and the branch cut.} Fractional powers are the
one class in which the choice of integration line is a question of validity
rather than of cost. The principal branch of $\lam^{\mu}$ is analytic on the
complex plane cut along the negative real axis, and $\Dfun$ inherits that cut
(measured across it, $\Dfun$ jumps by $\FracJump$ for the fractional
controller of Appendix~\ref{app:gallery}, while across the imaginary axis the
jump is identically zero). Since the cut lies entirely outside the closed
right half-plane, the choice $\sigma=0$ is admissible -- which is what makes
the whole class available to the method. The branch \emph{point} at $\lam=0$
lies on the contour, but the resulting integrand singularity
$\sim\omega^{\mu-1}$ is integrable for $\mu>0$ and contributes nothing
provided $\Dfun(0)\neq0$; when $\Dfun(0)=0$ the origin is itself a root on
the contour and the package reports the invalid marker of
Section~\ref{sec:integerness} rather than inventing a number. A
\emph{shifted} line ($\sigma<0$), however, encloses the segment $[\sigma,0]$
of the cut, the argument principle does not apply, and
Appendix~\ref{app:gallery} demonstrates what that costs.
...
with $\sigma<0$, however, encloses part of the cut, so the argument principle
does not apply there; the fractional case studies of
Appendix~\ref{app:gallery} show how the violation announces itself.
